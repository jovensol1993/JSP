...
//			ds = (DataSource) context.lookup("java:/comp/env/jdbc/TestDB");
			ds = (DataSource) context.lookup("java:/comp/env/jdbc/FreeDB");
			// 찾을 이름 작성 규칙
			// 	java:/comp/env/찾을이름
			// 따라서
	...
	*/
	...
	public Connection getCon() { //Connection : DB와 연결하는 객체
		Connection con = null;
		try {
	...
	// Statement 반환해주는 함수 , Statement : java에서 DB로 Query 문 전달 해주는 인터페이스
